...
Replace-TextInFile "$PSScriptRoot\OsuHelper\Properties\AssemblyInfo.cs" '(?<=Assembly.*?Version\(")(.*?)(?="\)\])' $newVersion
Replace-TextInFile "$PSScriptRoot\Deploy\Choco\osuhelper.nuspec" '(?<=<version>)(.*?)(?=</version>)' $newVersion
